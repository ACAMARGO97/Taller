...
export class CreateVentasComponent implements OnInit {
  ...
  onSubmit():void{
    ...
    this.ventaService.create(formValue)
    .subscribe({
      next:(res: any) => {
        this.snackBar.open('Ventas Creadas Correctamente.. !!','OK', {duration:500});
        this.router.navigateByUrl('/ventas');
      },
      error:(err: any) => {
        this.snackBar.open('Error al crear la Venta.. !!','ERROR', {duration:500});
        console.log(err)
        this.router.navigateByUrl('/ventas');
      }
    })
